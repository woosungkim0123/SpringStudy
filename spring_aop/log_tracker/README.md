...
- `ThreadLocal`은 각 스레드가 자신만의 독립적인 데이터를 보유할 수 있게 해주는 메커니즘입니다.
- `ThreadLocal`은 스레드가 데이터를 저장하고 접근할 수 있는 고유한 "접근 방법"을 제공하는 것이지 저장소 자체가 아닙니다.
- `ThreadLocal`을 사용하면, 한 스레드 내에서 설정된 데이터는 해당 스레드에서만 접근 가능하며, 다른 스레드와는 데이터가 공유되지 않습니다.
